...
`.trim(), data.jid)
            ...
            if ((usedPrefix = (match[0] || '')[0])) {
                ...
                if (!isPrems && plugin.limit && global.db.data.users[m.sender].limit < plugin.limit * 7) {
                    this.reply(m.chat, `Yah limit Kamu sudah melebihi Kuota 🤓, Beli limit Dengan Cara *${usedPrefix}buy limit*`, m)
...
let mssg = {
        rowner: 'Fitur khusus owner, *User* tidak dapat mengakses nya :!',
        owner: 'Fitur khusus owner, *User* tidak dapat mengakses nya :!',
        mods: 'Fitur Ini Khusus Moderator'}[type]
        
        if (mssg) return conn.sendMessage(m.chat, {
      text: mssg, 
      contextInfo: {
      externalAdReply: {
...
let msg = {
        ...
        restrict: 'Restrict Not Turned On For This Chat'}[type]
  ...
  if (msg) return conn.sendMessage(m.chat, {
    ...
    let daftar = {
  unreg: '_Untuk dapat mengakses fitur Bot,_\n_Anda harus melakukan *daftar* terlebih dahulu._\n_Untuk melakukan *daftar* caranya cukup mudah_\n_ketik:_\n_#daftar nama.umur_'}[type]
  
  if (daftar) return conn.sendMessage(m.chat, {
      text: daftar, 
      contextInfo: {
      externalAdReply: {
      title: '✖️ N O T  R E G I S T E R',
