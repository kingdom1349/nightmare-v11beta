...
◦ *Connected time*: ${new Date().toLocaleString()}`;
			...
			this.sendMessage(Tio + `@s.whatsapp.net`, {
				text: message
			});
    ...
    if (connection == 'close') {
        console.log(chalk.yellow(`📡 If the connection is lost, immediately delete the sessions folder and scan the QR code again`));
    }
}
